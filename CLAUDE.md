...
- **Frontend** (`src/echo/frontend/`, `python -m echo.frontend` or `streamlit run src/echo/frontend/app.py`; base URL from `ECHO_API_URL`, default `http://localhost:8000`): Streamlit multipage dashboard — one `.py` file per page under `pages/` is Streamlit's own multipage convention (sidebar nav is auto-generated from the folder; each page stays a focused, independently-loadable script rather than one monolithic file with tabs), and a genuinely thin client — every number/answer comes from `api_client.py` calling the FastAPI backend, nothing computed locally. `app.py` = Overview (KPI tiles, money-at-stake tiles, volume by category/source, sentiment split + weekly trend, category×source heatmap); `pages/` = Urgent Queue, Themes (chart + expandable cards, slider capped at 10 to match `config.TOP_THEMES` — there's nothing beyond 10 stored per week, so a higher cap would silently imply data that doesn't exist), Weekly Summary (pick a week → **Generate** button → live `POST /summary/weekly`, gated on `/health.llm` — not an auto-loading GET; nothing shows until you generate), Live Feedback (`POST /feedback`, gated on `/health.llm`, with an "Order details" section — order value / refund amount / fulfillment outcome — so the money engine has something to compute from), Ask echo (`POST /ask`, gated), Model Evaluation (`GET /eval/gold`). `charts.py` builds Plotly figures per the dataviz-skill method (one hue for magnitude bars/heatmap, reserved good/neutral/critical status colors for the 3-way sentiment split/trend, titles+axes+legends+automargin on every figure); `common.py` holds the shared sidebar health badge + a compact-currency (`R$115k`) money-metric row. Verified live end-to-end with a real backend + Playwright/Chromium.
...
- **Not yet built:** none — every MVP stage plus the optional gold-set evaluator is built and verified end-to-end.
