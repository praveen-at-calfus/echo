...
**Gotcha (bit us once, 2026-07-28): local Postgres + the Docker `db` container both default to host port 5432 — whichever grabbed it first wins, silently.** If DB-dependent things look stale/wrong (e.g. a page showing old data after a fix), check `docker compose ps` / `lsof -iTCP:5432` before assuming the code is broken — you may be looking at the *other* instance. Same applies to 8000/8501: `docker compose up` and a local `python -m echo.api` / `streamlit run` fight over the same ports, and whichever is already listening wins. After any Docker packaging change, remember the running container's Postgres **volume** doesn't pick up a regenerated `docker/seed/*.sql.gz` on its own — `docker-entrypoint-initdb.d` only runs on an empty volume, so a code/data fix needs `docker compose down -v && docker compose build && docker compose up -d` to actually land in a container someone's been viewing.
