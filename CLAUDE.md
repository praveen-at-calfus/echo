...
**Gotcha (bit us once, 2026-07-28): local Postgres + the Docker `db` container both default to host port 5432 — whichever grabbed it first wins, silently.** If DB-dependent things look stale/wrong (e.g. a page showing old data after a fix), run **`./scripts/status.sh`** before assuming the code is broken — you may be looking at the *other* instance (it can even report a genuine `CONFLICT` — Docker Desktop's macOS port-forwarding proxy doesn't always collide with a plain local bind the way you'd expect, so both can be listening on 5432 at once). Same applies to 8000/8501: `docker compose up` and a local `python -m echo.api` / `streamlit run` fight over the same ports. After any Docker packaging or data change, the running container's Postgres **volume** doesn't pick up a regenerated `docker/seed/*.sql.gz` on its own — `docker-entrypoint-initdb.d` only runs on an empty volume — so use **`./scripts/refresh-docker.sh`** (regenerates the seed from local Postgres, rebuilds, `down -v`, `up -d`) rather than doing that 4-step dance by hand; it refuses to run if port 5432 isn't unambiguously your local Postgres, so it can't accidentally dump from stale Docker data back into the seed.
